...
public enum Config implements PeConfig {
  ...
  private final static String SERVICE_ID = "pe/bank.processor.flink";

  Config() {
    config = SpConfig.getSpConfig(SERVICE_ID);
    config.register(ConfigKeys.HOST, "FT", "Data processor host");
    config.register(ConfigKeys.PORT, 8090, "Data processor port");
    config.register(ConfigKeys.SERVICE_NAME, "FT", "Data processor service name");
